...
public class ContactController {
    ...
    public ResponseEntity<?> submitContact(@RequestBody Map<String, String> body) {
        ...
        log.info("📩 [Contact API] Nhận tin nhắn liên hệ mới từ {}: {} ({}) - Chủ đề: {}", fullname, email, phone, subject);

        if (mailSender != null && email != null && !email.isBlank()) {
            // Send email asynchronously in background thread so HTTP response returns immediately (< 50ms)
            CompletableFuture.runAsync(() -> {
                try {
                    // 1. Send notification email to Support Desk (homixspace@gmail.com)
                    ...
                    log.info("✅ [Contact Mail] Đã gửi email thông báo tới bàn CSKH ({})", fromEmail);
                    ...
                    log.info("✅ [Contact Mail] Đã gửi email xác nhận THÀNH CÔNG cho khách hàng {}", email);
                } catch (Exception e) {
                    log.error("❌ [Contact Mail Exception] Lỗi gửi email liên hệ qua Gmail SMTP: ", e);
                }
            });
        } else if (mailSender == null) {
            log.warn("⚠️ JavaMailSender is NULL! Mail service is not configured.");
        }
